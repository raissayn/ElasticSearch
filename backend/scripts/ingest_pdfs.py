...
def main() -> int:
    ...
    parser.add_argument(
        "--dir",
        help="Directory containing PDF files to ingest automatically (ignores manifest if provided).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.dir:
        ...
        for pdf_file in dir_path.glob("*.pdf"):
            source_id = pdf_file.stem.lower().replace(" ", "_").replace("-", "_")
            sources.append({
                "source_id": source_id,
                "title": pdf_file.stem,
                "local_path": str(pdf_file),
                "public_url": f"/{pdf_file.name}",
                "doc_type": "documento",
            })
        print(f"Found {len(sources)} PDFs in {dir_path}")
    else:
